...
namespace EnrollmentSystem
{
    public partial class Transferee : Form
    {
        ...
        {
            if (MessageBox.Show("YOU WANT TO EXIT?", "Transferee Registration", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                new Dashboard().Show();
                this.Hide();
            }
        ...
        private void btnClickSave(object sender, EventArgs e)
        {
            ...
            PrevSchool, ContactNum, EmailAdd, DOB) VALUES (@StudentID, @FirstName, @MiddleName, @LastName, @GradeLevel, @Sex, @Strand, @Address, 
            @PrevSchool, @ContactNum, @EmailAdd, @DOB)", con);
